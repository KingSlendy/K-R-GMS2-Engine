...
function dynamic_collision(setup = false) {
	if (argument_count >= 2) { 
		var script = argument[1]; //Script to run when the instance is crushed at the end of collision
	}

	if (argument_count >= 3) { 
		var arg = argument[2]; //Argument for crush script
	}
